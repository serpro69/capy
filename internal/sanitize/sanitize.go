...
var secretPatterns = []*regexp.Regexp{
	...
	regexp.MustCompile(`(?i)(?:api[_-]?key|secret|token|password|credential|auth)\s*[=:]\s*["']?[A-Za-z0-9_\-/.+]{20,}["']?`),
}
...
func StripSecrets(content string) string {
	...
	if strings.Contains(content, "<private>") || strings.Contains(content, "<PRIVATE>") || strings.Contains(content, "<Private>") {
