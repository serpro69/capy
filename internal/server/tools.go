...
func toolFetchAndIndex() mcp.Tool {
	return mcp.NewTool("capy_fetch_and_index",
		...
		mcp.WithString("url",
			mcp.Description("The URL to fetch and index. Provide this (single-URL mode) OR requests (batch mode)."),
		),
		mcp.WithString("source",
			mcp.Description("Label for the indexed content"),
		),
		mcp.WithArray("requests",
			mcp.Description("Batch mode: array of {url, source?} objects fetched concurrently and indexed together. Alternative to url+source — use with concurrency for I/O-bound multi-source research. Each result's preview is capped at 384 chars."),
			mcp.Items(map[string]any{
				"type": "object",
				"properties": map[string]any{
					"url":    map[string]any{"type": "string", "description": "URL to fetch and index"},
					"source": map[string]any{"type": "string", "description": "Label for the indexed content"},
				},
				"required": []any{"url"},
			}),
		),
		mcp.WithNumber("concurrency",
			mcp.Description("Batch mode only: parallel fetch workers (1-8). Default 1 = serial. Use 4-8 for I/O-bound multi-URL fetches."),
			mcp.Min(1),
			mcp.Max(8),
		),
		...
		mcp.WithString("kind",
			mcp.Description("Source kind for indexed content: 'ephemeral' (default, 24h TTL, excluded from default search) or 'durable' (retained by retention score, included in default search). Use 'durable' for reference docs you want to persist across sessions. Applies to all URLs in batch mode."),
