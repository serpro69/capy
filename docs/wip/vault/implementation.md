...
> **v1 build reality (Task 6).** The viewer ships the **markers-only** fallback (below), and three further deliberate deviations are tracked as non-blocking follow-ups in [tasks.md](./tasks.md): (1) **eager render-once** instead of the lazy `\n`-windowed renderer (the search→scroll `line_index`/`subagent_id` anchor contract is preserved via a source-line→display-row map; the lazy + no-dedup combination would re-show progressive snapshots, and the eager cost equals `show`'s); (2) **mode-based single-pane** navigation instead of the side-by-side split in §Layout; (3) marker→subagent opening uses an **order-based launch mapping** (openable only when counts align), since the JSONL has no verified `tool_use`↔`agent-<id>` link — search-jump stays exact. Parsing lives in `internal/vault/transcript.go` (`ParseTranscript`, package `vault`); `tui/render.go` does lipgloss styling + the row map.
>
