...
**`capy vault rekey` (V2.8):** prompt for the current passphrase, read the new one from `CAPY_VAULT_KEY` (the new key must already be exported, consistent with `capy encrypt`), call `sqliteutil.Rekey(VaultDBPath(), old, new)`. **Do not run a `VaultStore.Checkpoint()` pre-check** — `Checkpoint()` opens the DB with `CAPY_VAULT_KEY`, which now holds the *new* key, so it cannot open a DB still encrypted with the *old* key and would fail before rotation begins (corrected after review). The checkpoint that flushes the WAL happens *inside* `Rekey`, on the source connection opened with the **old** key (`rekeyEncrypted`'s `checkpointDB(srcDB)`), so no separate pre-check is needed.
...
1. **Archive the active session first** by running the existing single-session import path for that one session (`DiscoverSessions` for the dir → filter to this UUID → `Import`). This reads the pre-compaction **main + sidecars** from disk, applies the normal idempotent decision, and *creates the parent `vault_sessions` row if absent*. Reusing `Import` (not a bespoke main-only upsert) avoids two bugs: it satisfies the snapshot FK, and it never clobbers a larger main+sidecar active row with a main-only blob (a `ReplaceSession` built from a sidecar-less record would `DELETE` then fail to re-insert the sidecars).
2. **Then insert the snapshot** (`InsertSnapshot`, dedup via UNIQUE) of the main transcript.

Hooks are short-lived processes (CLAUDE.md invariant): open vault → import one session + insert snapshot → checkpoint/close, fast and synchronous, so the snapshot is durable before compaction proceeds. Failures are logged, never block the user's `/compact`. Test explicitly with **no pre-existing parent row**.
...
- **`context.Context` propagation, both stores (V2.2a store, V2.2b vault):** per the user's explicit decision (sibling consistency over divergence), convert `internal/store` and `internal/vault` to `*Context` variants (`QueryContext`/`ExecContext`/`BeginTx`) and add `ctx` params to public methods, in **two behavior-preserving commits** (store, then vault). **Honest rationale (corrected after review — the earlier "V2.10 justifies it" claim was misleading):** this is a **consistency-only refactor with near-zero functional payoff today.** `VaultStore.ctx()` returns `context.Background()` (`store.go:200`), and the one cancellable operation — the all-projects sweep (V2.10) — lives in `internal/vault` and *already* gets cooperative cancellation from `Import`'s per-session loop check (`import.go:136`); it does **not** depend on store-side ctx, and vault's batched writes are short, not long-running transactions worth interrupting mid-flight. So **V2.2a (store) has no functional beneficiary at all** — it spends regression risk on encryption-critical code (CLAUDE.md's foremost invariant) purely for symmetry. The `Task 4 → Task 3` dependency is **sequencing preference, not a code dependency** (vault does not consume store's ctx). **Recommendation (flagged to the user): do V2.2b (vault) and defer V2.2a (store) until a store-side cancelling caller exists** — "no diff is the safest diff" on encryption paths. Retained here only because the user chose full consistency; revisit before implementing.
...
2. **`merge` source open:** ~~open question~~ **RESOLVED** — a dedicated `sqliteutil.OpenReadOnly` that does **not** run schema DDL/migrations against the source, and **checkpoints any pending WAL first** (folding a copied-live vault's `-wal` into the main file) rather than `immutable=1` (which would silently skip WAL-resident rows). See §3.
