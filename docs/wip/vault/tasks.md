...
### Subtasks
- [ ] 2.1 Create `internal/vault/scanner_types.go` — minimal JSON wire types: `jsonlLine` (with `cwd`, `gitBranch`, `aiTitle`, and `prUrl`/`prRepository`/`prNumber` fields; **no `customTitle`** — deferred tier), `jsonlMessage`, `contentBlock` (decoupled from `internal/session/parse.go`)
