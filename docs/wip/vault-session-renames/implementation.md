...
3. Add `Name string` (or equivalently explicit name-filter state) to `ListOptions`. Apply a parameterized, case-insensitive literal substring predicate to the effective title, combine it with project filtering by AND, order newest-first, then limit. Do not interpolate query text or interpret FTS/wildcard syntax. → verify: tests cover mixed case, literal `%`/`_`/quotes, combined project/name filtering, no match, and limit-after-filter.
...
1. Add a shared name normalizer in `internal/vault/`: trim, strip recognized secrets, reject post-normalization empty values, reject Unicode control characters, and reject values over 120 runes. Keep clear represented separately from an empty string. → verify: table tests cover Unicode length boundaries, whitespace, controls, secret redaction, and duplicate-name acceptance.
...
4. Exercise concurrent rename/delete and repeated rename behavior under the race detector. The transaction must yield either a completed rename on an existing session or an actionable error, never orphan metadata. → verify: focused `go test -race -tags fts5` cases pass repeatedly.
...
2. Implement a conditional name upsert comparing `(renamed_at_ns, machine_id)`. Null titles must remain tombstones. Keep the comparison independent of transcript content decisions. → verify: a table-driven matrix covers newer/older timestamp, equal timestamp machine tie-break, named/cleared combinations, and identical state.
3. Integrate reconciliation into `MergeFrom`: insert a new session plus name atomically; allow name-only updates when equal-hash or smaller source transcripts are skipped; report name-only change as updated; preserve per-session error continuation. → verify: merge tests cover all three transcript branches, source-only sessions, write errors, and destination transcript immutability.
4. Apply the same decision in dry-run without writes and surface the prospective effective title. Repeat each merge fixture to prove idempotence and run source/destination order checks for timestamp ties. → verify: second runs report skipped and both merge directions converge on the same effective title.
...
2. Route `e` from list, search result, and viewer to a prefilled `name (empty clears)` input. While editing, consume keys before normal mode routing; `Enter` submits asynchronously, `Esc` cancels, and a pending request ignores duplicate submit. → verify: model tests cover all entry modes, prefill, cancel, empty clear, and pending-state behavior.
...
4. Broaden list `f` filtering to effective title, project path, and UUID while leaving `/` as transcript FTS search. Render effective titles in search results and add `e rename` to mode help. Preserve `r`/`R` restore/resume and viewer `n` marker navigation. → verify: keybinding/filter tests cover all searchable fields and existing navigation suites remain green.
...
3. Run formatting, focused package/CLI tests, `make vet`, `make test`, and `make test-race` with required keys/tags. Retrieval benchmarks are intentionally skipped because no indexed content or ranking changes. → verify: all commands exit successfully with no race reports.
...
- Machine clocks are sufficiently aligned for latest recorded timestamp to represent user intent.
- Exact same-machine/same-nanosecond divergent writes do not arise through supported commands.
- Duplicate names are acceptable; UUID prefixes remain the mutation handle.
- Expected vault size makes effective-title substring scans acceptable.
