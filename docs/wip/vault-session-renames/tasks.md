...
- [ ] 1.5 Add focused race coverage for rename/delete and repeated concurrent rename behavior
...
- [ ] 2.2 Add literal `ListOptions` name filtering and expose it as `vault list --name`; combine it with project filtering and return effective titles in table/JSON output
...
- [ ] 3.2 Add latest-`(renamed_at_ns, machine_id)` conditional reconciliation with null clear tombstones
- [ ] 3.3 Integrate atomic new-session+name writes and name-only updates for same-hash/smaller-transcript branches; make dry-run/status output reflect effective results
- [ ] 3.4 Extend `internal/vault/merge_test.go` with newer/older/tie/tombstone, transcript-branch, legacy, failure, dry-run, convergence, and idempotence cases
...
- [ ] 4.2 Bind `e` in list/search/view, prefill the effective title, treat empty submit as clear, retain input on errors, and suppress duplicate pending writes
...
- [ ] 4.4 Broaden `f` filtering to effective title/project/UUID, render titles in search rows, and update help without changing `r`/`R` or viewer marker navigation
- [ ] 4.5 Add TUI model/keybinding tests for all entry modes, success, clear, cancel, error, pending, filtered disappearance, refresh, and existing navigation regressions
