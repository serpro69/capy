...
- **Docs:** [implementation.md#task-1--genericinputsummary--wire-into-tooluse­summary](./implementation.md)
...
- [ ] 1.5 Table-driven tests for `genericInputSummary` in `scanner_test.go`: priority+alphabetical selection surfaces `queries`/`source` for a `capy_search`-shaped input; `+N` marker on overflow; array-of-strings inline; nested object → `{…}` (contents absent); long key truncated; embedded-newline value → single line; empty/non-object/malformed → bare name; `Bash` regression guard
