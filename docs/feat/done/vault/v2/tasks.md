...
- [x] 0.3 Document the JSON shape in `docs/feat/wip/vault/v2/precompact-investigation.md`: is the session file path present? session ID? project dir? — yes to all (`transcript_path`, `session_id`, `cwd`, `hook_event_name`, `trigger`)
...
- **Status:** done (17.1–17.4 complete; 2026-06-29)
- **Depends on:** Task 1, 2, 4–13 (Tasks 3, 14–16 dropped)
- **Size:** S
- **Can run in parallel with:** —

### Subtasks
- [x] 17.1 Run `/kk:test` — full suite (`-tags fts5`, both keys, race); no regressions in existing tests — **green.** `go test -race -tags fts5 ./...` PASS (exit 0, zero FAIL); `go vet -tags fts5 ./...` + `go vet -tags fts5,glamour ./internal/vault/tui/...` clean; glamour-tagged tui race tests PASS; linkage guard holds (default binary 0 glamour symbols, `build-glamour` 152). Active profile: `go` (extension-authoritative).
- [x] 17.2 Run `/kk:document` — update `docs/architecture.md`, `CLAUDE.md`, AND `README.md` (new subcommands `compact`/`merge`/`rekey`, `-tags glamour` build, `CAPY_VAULT_SWEEP_ALL`/`CAPY_VAULT_MERGE_KEY` env vars) — **done.** README: compact/merge commands + merge cross-machine workflow (replaces stale "planned for a future version"), glamour build note, env-vars table gains `CAPY_VAULT_SWEEP_ALL`/`CAPY_VAULT_MERGE_KEY`/`CAPY_VAULT_NO_COMPRESS`, all-projects sweep on the startup-sweep bullet, and the stale "Compaction gap (v1)" note corrected to the append-only finding (per precompact-investigation §0.4/0.6). architecture.md: new "Blob compression & storage encoding" subsection, schema `encoding`/`min_reader_version`, archival-paths sweep-all + merge, CLI table compact/merge/rekey, glamour `renderBody` seam. CLAUDE.md: vault encoding-authoritative + rekey-backup-API invariants, `make build-glamour` build/test targets. CLI rubric (cobra) applied; no new ADR (decisions captured in vault v2 design + `kk:arch-decisions`).
- [x] 17.3 Run `/kk:review-code go` — review the full v2 diff — **done.** Profile `go`; checklists loaded: solid, security(+injection), error-handling, code-style, naming, database, concurrency. Reviewed the full diff (5,714 ins / 57 files) with focus on the high-risk surfaces (codec, compact, merge, `sqliteutil.Rekey`/`OpenSourceForMerge`, exec of `claude`, OSC-52, migration 0001, diff reconstruction). **No P0/P1/P2 findings** — every task's prior isolated review (code-reviewer + pal) shows: SQL fully parameterized (dynamic `encCol`/table names are `//nolint:gosec`-annotated trusted constants), errors wrapped with `%w`, dual-`%w` rekey rollback, exec uses `LookPath`+separate args (no shell), Close-before-exec ordering, defensive diff parsing, cooperative-cancellation sweep. **P3 (style):** one v2-introduced gofmt regression in `import.go` (`ftsBatch[i : i+1]`) — **fixed** (`gofmt -w`). No P0/P1 systemic findings → `kk:review-findings` indexing skipped per skill.
  - **DEFERRED (user decision, 2026-06-29):** 28 pre-existing gofmt-dirty files across 8 packages (adapter, executor, hook, security, server, session, store, vault) — all dirty on `master`, NOT v2-introduced, so out of v2 scope. User opted to format later. **Root cause:** no CI gofmt gate — `make fmt`/`go fmt` is a manual target only. **Next step:** run `gofmt -w .` as its own dedicated commit (separate from feature work) AND add a `gofmt -l` (fail-if-nonempty) check to `.github/workflows/` so unformatted code fails CI and stops re-rotting.
- [x] 17.4 Run `/kk:review-spec` — verify implementation matches design.md + implementation.md — **done.** Profile `go`; post-implementation full-feature scope. **Implemented features all conform to spec** (codec/encoding + `min_reader_version`, compact, Rekey extraction, vault rekey, merge, all-projects sweep, 0-msg exclusion, TUI keys, glamour build tag, A1–A4) — verified against the code read in 17.3. **All code deviations are intentional + recorded in tasks.md** (SPEC_DEV: `buildFTS`→`scanSessionAndSubagents` naming [9.2], glamour v0.9.1 not literal-latest [13.1], `sessionMetaColumns` kept metadata-only [5.5]; EXTRA: `OpenSourceForMerge` +ctx/+keyEnv [9.1], `EncryptedDSN`/`Checkpoint`/`SwapAndVerify` moved to sqliteutil [7]) — no unconfirmed deviations to index. **One OUTDATED_DOC finding (P3), fixed:** design.md + implementation.md still described the dropped snapshot/PreCompact machinery in their summary/detail sections (Architecture deltas, Schema changes, §3 merge snapshot-carry, impl §Durability + V2.9) though the drop was recorded in the §5 banner + tasks.md — added surgical DROPPED markers so the planning docs no longer contradict themselves. No P0/P1/P2; no MISSING_IMPL (dropped tasks are intentional, not gaps).
