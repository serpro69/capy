...
Two tables + 1 FTS5 virtual table + 1 metadata table + 1 index.
...
`idx_sessions_end_time` supports the `list` command's default `ORDER BY end_time DESC`. **No `project_path` index is created:** `--project` is a *substring* match (`WHERE project_path LIKE '%…%'`), which a B-tree index can't accelerate (leading wildcard), and a full scan over a single-user vault (hundreds–low-thousands of sessions) is cheap. Because location is 1:1 on `vault_sessions`, `list` is a plain `SELECT ... ORDER BY end_time DESC` (no `GROUP BY`), and `--project` on both `list` and `search` is a direct `WHERE project_path LIKE ?` (no `EXISTS` subquery, no row multiplication to guard against).

**`vault_meta`** — key-value store for vault-level configuration. Reserved for future use (e.g. a format marker); **migration state is tracked in `vault_migrations`, not here.**
...
- **`subagent_id` uses empty string sentinel** — main-session rows store `''` (not NULL) so that filtering and grouping by `subagent_id` use plain equality (`WHERE subagent_id = ''`) rather than SQLite's three-valued NULL logic (`IS NULL`), keeping `vault_fts` queries uniform. (NULL would also misbehave in any future composite key over the column.)
...
- **`claude_project_dir`** — the mangled directory name (e.g., `-home-sergio-Projects-personal-capy`), always populated. For the standard walk of `~/.claude/projects/` it's the real mangled dir; for a non-standard `--path` import of *loose* JSONL files (not under a `projects/<mangled>/` dir) it's a best-effort placeholder (the containing dir's basename), so restore-to-default may target the wrong path — use `--output` for those. Used for restore (writing back to `~/.claude/projects/<claude_project_dir>/<uuid>.jsonl`).
