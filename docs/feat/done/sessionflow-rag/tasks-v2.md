...
- [x] 0.1 Capture v1 baseline: run `go run -tags fts5 ./cmd/dump-session/ .capy/knowledge.db b73c8a63 > docs/feat/wip/sessionflow-rag/comparison-baseline-session.md` to dump all chunks for session `b73c8a63` (the v2 design session). This file is consumed by Task 5 for the two-lens comparison. This MUST run before any code changes to capture the v1 index state.
...
- [x] 4.3 Expose `capy sweep --reindex` CLI flag that calls `SweepWithOptions` with `Reindex: true`. The existing `SweepOptions.Reindex` field already bypasses the mtime gate — just needs CLI wiring. _(Pre-existing in `cmd/capy/sweep.go`)_
- [x] 4.4 Add test: `capy sweep --reindex` re-parses sessions regardless of mtime. Without flag, unchanged sessions are skipped. _(Pre-existing: `TestSweepWithOptions_Reindex`, `TestDryRunSweep_AlreadyIndexed`)_
...
- [x] 5.2 Capture v2 results: run `go run -tags fts5 ./cmd/dump-session/ .capy/knowledge.db b73c8a63 > docs/feat/wip/sessionflow-rag/comparison-baseline-session-v2.md` to dump the re-indexed session. Compare against `comparison-baseline-session.md` from Task 0.
...
- [x] 5.5 Feature usefulness. Discover some decisions from the baseline session via capy search. Is it possible to discover past information from sessions? Is that information accurate? Is it useful? Rate the overall feature usefulness.
...
- [x] 6.1 Run `test` skill — full test suite (`make test`, `make test-race`), verify no regressions
- [x] 6.2 Run `document` skill — update CONTRIBUTING.md if the `tools.go` extractor pattern is worth documenting
- [x] 6.3 Run `review-code` skill with Go input to review the implementation
- [x] 6.4 Run `review-spec` skill to verify implementation matches design-v2.md and implementation-v2.md
- [x] 6.5 Delete `cmd/dump-session/` — temporary utility, not part of the shipped CLI. Already deleted in prior session.
